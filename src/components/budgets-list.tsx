        ...
        <div className="space-y-6">
          {budgets.length === 0 ? (
            ...
            budgets.map((budget) => {
              ...
              return (
                <div key={budget.id} className="space-y-3 p-4 rounded-lg bg-muted/50">
                  ...
                  <div className="space-y-2">
                    ...
                    <Progress
                      ...
                      className={`h-2 ${isOverBudget ? "[&>div]:bg-destructive" : isNearLimit ? "[&>div]:bg-accent" : ""}`}
                    />
                    <div className="flex items-center justify-between text-xs">
                      <span
                        className={`font-medium ${
                          isOverBudget ? "text-destructive" : isNearLimit ? "text-accent" : "text-muted-foreground"
                        }`}
                      >
                  ...
                  {isNearLimit && !isOverBudget && (
                    <div className="text-xs text-accent bg-accent/10 p-2 rounded">
                      Approaching budget limit - {(100 - percentage).toFixed(0)}% remaining
                    </div>
                  )}
                </div>
              )
